...
# Construct a version to be whatever is in the package.json + the build info
NPM_VERSION=$(cat package.json | jq '.version' | sed 's/"//g')"-"$(echo $BUILD_VERSION | sed 's/[_-+ ]//g')
# Update the version in the package.json. Revert to original version first in case this script is run twice.
git co -- package.json
