...
class CONFIG:
    IS_LOCAL_SERVER = (
        True  # If False consider that uploaded files are relatives and can have dangerous paths.
    )
    ...
    def set_config(config):
        ...
        return (
            ...
            + ", EVENTS_STRING: "
            + str(CONFIG.EVENTS_STRING)
            + ", GTI_STRING: "
            + str(CONFIG.GTI_STRING)
            + ", FRACEXP_LIMIT: "
            + str(CONFIG.FRACEXP_LIMIT)
            + ", USE_JAVASCRIPT_CACHE: "
            + str(CONFIG.USE_JAVASCRIPT_CACHE)
        )
