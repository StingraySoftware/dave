...
class TestTimingAnalysisValidation:
    ...
    def synthetic_lightcurve_file(self, client):
        ...
        signal = (10.0 +
                 2.0 * np.sin(2 * np.pi * freq1 * times) +
                 1.0 * np.sin(2 * np.pi * freq2 * times))
    ...
    def test_power_density_spectrum_peaks(self, client, synthetic_lightcurve_file):
        ...
        if 'data' in result and 'pds' in result['data']:
            ...
            assert freq1_found or freq2_found, f"Expected frequencies {freq1}, {freq2} Hz not found in peaks at {peak_freqs}"
    ...
    def test_phase_lag_calculation(self, client, synthetic_lightcurve_file):
        ...
        response = client.post('/get_phase_lag_spectrum',
                             data=json.dumps(params),
                             content_type='application/json')
    ...
    def test_rms_spectrum_variability(self, client, synthetic_lightcurve_file):
        ...
        response = client.post('/get_rms_spectrum',
                             data=json.dumps(params),
                             content_type='application/json')
        ...
        if ('success' in result and result['success'] and
            'data' in result and 'rms' in result['data']):
    ...
    def test_pulse_search_detection(self, client):
        ...
        try:
            ...
            response = client.post('/get_pulse_search',
                                 data=json.dumps(params),
                                 content_type='application/json')
            ...
            if ('success' in result and result['success'] and
                'data' in result and 'freq' in result['data']):
...
class TestSpectralAnalysisValidation:
    ...
    def test_cross_spectrum_coherence(self, client):
        ...
        phase_shift = np.pi / 4
        signal2 = 100.0 + 8.0 * np.sin(2 * np.pi * common_freq * times + phase_shift) + 5.0 * np.random.randn(len(times))
        ...
        if ('success' in result and result['success'] and
            'data' in result and 'coherence' in result['data']):
            # Cross spectrum should show high coherence at common frequency
                ...
                assert coh_at_freq > 0.5, f"Expected high coherence at {common_freq} Hz, got {coh_at_freq}"
    ...
    def test_covariance_spectrum(self, client):
        ...
        data_lines = ["# Multi-band lightcurve\n",
                      "# Column 1: TIME\n",
                      "# Column 2: RATE\n",
                      "# Column 3: ERROR\n",
                      "# Column 4: SOFT_RATE\n",
        ...
        for t in times[:100]:  # Use subset for speed
            ...
            data_lines.append(f"{t:.3f} {total:.1f} {np.sqrt(total):.1f} {soft:.1f} {np.sqrt(abs(soft)):.1f} {hard:.1f} {np.sqrt(abs(hard)):.1f}\n")
        ...
        try:
            ...
            response = client.post('/get_covariance_spectrum',
                                 data=json.dumps(params),
                                 content_type='application/json')
