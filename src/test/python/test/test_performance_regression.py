...
from utils.dataset_helper import get_filtered_dataset
...
class TestDatasetPerformance(PerformanceBenchmark):
    ...
    def test_filter_performance(self):
        ...
        for i, config in enumerate(filter_configs):
            # Time measurement
            time_taken, filtered = self.measure_time(
                apply_filters, dataset, config["filters"]
            )
...
class TestRegressionSuite:
    ...
    def test_full_analysis_pipeline(self):
        ...
        dataset.tables["EVENTS"] = Table("EVENTS")
        dataset.tables["EVENTS"].columns["TIME"] = Column("TIME", time)
        dataset.tables["EVENTS"].columns["PI"] = Column(
            "PI", np.random.randint(20, 200, size)
        )
        ...
        counts, _ = np.histogram(
            filtered.tables["EVENTS"].columns["TIME"].values,
            bins=bins
        )
